...
var createPreprocesor = function(logger, config, basePath) {
  ...
  var shouldExecFile = config.shouldExecFile;
  var processContent = config.processContent;
  ...
  return function(content, file, done) {
    ...
    if (processContent) {
      log.debug('calling process content for "%s".', file.originalPath);
      processContent(content, file, function (content) {

        var output =
          'window.__cjs_module_aliases = ' + JSON.stringify(aliases) + '\n' +
        ...
        shouldExecFile && (output += 'window.__cjs_module__["' + file.path + '"].autoExecute = ' + (!!shouldExecFile(file)) + ';\n');

        done(output);
      });
    }
    //processContent && (content = processContent(content, file));
